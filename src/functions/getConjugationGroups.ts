...
import ConjugationTense from '../interfaces/ConjugationTense';

export default async function getConjugationGroups(verbQuery: string): Promise<ConjugationGroup[]> {

   const pageURL = `https://dictionnaire.lerobert.com/conjugaison/${encodeURIComponent(verbQuery)}`;
   ...
   const conjugationGroups: ConjugationGroup[] = [];

   for (const group of await page.$$('.conj')) {

      const groupName = await getElementProperty<HTMLHeadingElement, string>(
         await group.$('h2'),
         'textContent'
      );

      const conjugationGroup: ConjugationGroup = {
         name: groupName,
         subgroups: []
      }
      ...
      for (const subgroupNameElement of await group.$$('.cf-b')) {
         ...
         const subgroupName = await getElementProperty<Element, string>(subgroupNameElement, 'textContent');
